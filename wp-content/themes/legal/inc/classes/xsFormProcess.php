...
class xsFormProcess {
	...
	function add_branch() {
		...
		if ( $f->validate() ) {
			...
			if ( $branch_id > 0 ) {
				///
				$data = array( 'lat' => $_POST['xLat'], 'lng' => $_POST['xLng'] );
				global $wpdb;
	...
	function update_bus_profile() {
		...
		if ( $f->validate() ) {
			...
			if ( $_POST['xs_pwd'] != '' ) {
				wp_update_user( array( 'ID' => $user_id, 'user_pass' => $_POST['xs_pwd'] ) );
			}
	...
	function register() {
		...
		$f->setRule( 'xs_pwd', 'Password', 'required|callback[xs_pwd_match]' );
		//
		if ( ! in_array( $_POST['xs_user_type'], array( 'business', 'client' ) ) ) {
			...
			if ( $f->validate() ) {
				...
				if ( $xs_user->can_add_coupon( $credits_required ) || $function == 'wp_update_post' ) {
					...
					if ( $coupon_id > 0 ) {
						...
						// add category
						wp_set_post_terms( $coupon_id, array( $cat_id, $_POST['category'] ), 'coupons_category' );
						/////////////////////////////////////////////// create coupons
						if ( $function == 'wp_update_post' ) {
	...
	function send_love() {
		if ( ! is_user_logged_in() ) {
			...
			if ( $rows > 0 && is_numeric( $rows ) ) {
				...
				$data = array( 'lover_id' => get_current_user_id(), 'loved_id' => $_POST['uid'] );
