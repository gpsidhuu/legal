 ...
 * @author Gp
 * @link http://www.xtremestudioz.com
 * @version 0.0.1
 *
 **/
class xsValidate
{
    ...
    protected $_errorPhrases = array(
        ...
        'min_length' => '%1 must be at least %2 characters long.',
        'max_length' => '%1 must be no more than %2 characters.',
        'exact_length' => '%1 must be exactly %2 characters.',
        'matches' => '%1 must match the %2.',
        'valid_email' => '%1 must be a valid E-Mail Address.',
        'valid_number' => '%1 must be a valid number',
        'not_equal' => array(
            'post:key' => '%1 must not be the same as the %2 field.',
            'string' => '%1 must not be %2.'
        ),
        'depends' => '%1 depends on the %2 field being valid.'
    );
    ...
    protected $_allErrorsDelimiter = array(
        '<div class="alert alert-danger">',
        '</div>'
    );
    protected $_eachErrorDelimiter = array(
        '<p class="error">',
        '</p>'
    );
    ...
    public function setCustomError($inputName, $errorMessage)
    {
        ...
        $this->_errorSet[$inputName] = $errorMessage;
    ...
    protected function _validateDepends($inputName, $ruleName, array $ruleArgs)
    {
        if (array_key_exists($ruleArgs[1], $this->_errorSet)) {
            $this->_setError($inputName, $ruleName, array(
                $this->_getLabel($inputName),
                $this->_getLabel($ruleArgs[1])
            ));
        }
    }
        ...
        foreach ($canNotEqual as $doNotEqual) {
            ...
            if (preg_match('/post:(.*)/', $doNotEqual)) {
                if ($inputVal == $_POST[str_replace('post:', '', $doNotEqual)]) {
                    $this->_setError($inputName, $ruleName . ',post:key', array(
                        $this->_getLabel($inputName),
                        $this->_getLabel(str_replace('post:', '', $doNotEqual))
                    ));
                    continue;
                }
            } else {
                if ($inputVal == $doNotEqual) {
                    $this->_setError($inputName, $ruleName . ',string', array(
                        $this->_getLabel($inputName),
                        $doNotEqual
                    ));
                    continue;
    ...
    public function _validateValidnumber($inputName, $ruleName, array $ruleArgs)
    {
        $inputVal = $this->post($inputName);
